...
void XMLInterp4Config::ProcessLibAttrs(const xercesc::Attributes  &rAttrs)
{
 ...
 // Tu trzeba wpisać własny kod ...
	...
	_rConfig.AddLib(sLibName);

 // Tu się zakończyło wpisywanie własnego kodu...
...
void XMLInterp4Config::ProcessCubeAttrs(const xercesc::Attributes  &rAttrs)
{
 ...
 // Tu trzeba wstawić odpowiednio własny kod ...
	...
	auto getAttr = [&](const char* qname) -> std::string {
    		for (XMLSize_t i = 0; i < rAttrs.getLength(); ++i) {
      			char* an = xercesc::XMLString::transcode(rAttrs.getQName(i));
      			std::string name = an ? an : "";
      			xercesc::XMLString::release(&an);
      			if (name == qname) {
        			char* av = xercesc::XMLString::transcode(rAttrs.getValue(i));
        			std::string val = av ? av : "";
        			xercesc::XMLString::release(&av);
        			return val;
      			}
    		}
    		return {};
	...
	cube.Name = (sValue_Name ? sValue_Name : "");
	...
	if(!IStrm.fail()) {
		...
		std::string scaleStr = getAttr("Scale");
		if(!scaleStr.empty()) {
			std::istringstream is(scaleStr);
			double x,y,z;
			if(is >> x >> y >> z) { cube.Scale[0]=x; cube.Scale[1]=y; cube.Scale[2]=z;}

		}
	}

	// Shift (opcjonalny, domyślnie 0 0 0)
 	if (auto s = getAttr("Shift"); !s.empty()) {
		std::istringstream is(s);
		double x,y,z; if (is >> x >> y >> z) { cube.Shift[0]=x; cube.Shift[1]=y; cube.Shift[2]=z; }
	}

  	// RotXYZ_deg (opcjonalny, domyślnie 0 0 0)
  	if (auto s = getAttr("RotXYZ_deg"); !s.empty()) {
    		std::istringstream is(s);
    		double x,y,z; if (is >> x >> y >> z) { cube.RotXYZ_deg[0]=x; cube.RotXYZ_deg[1]=y; cube.RotXYZ_deg[2]=z; }
  	}

  	// Trans_m (opcjonalny, domyślnie 0 0 0)
  	if (auto s = getAttr("Trans_m"); !s.empty()) {
    		std::istringstream is(s);
    		double x,y,z; if (is >> x >> y >> z) { cube.Trans_m[0]=x; cube.Trans_m[1]=y; cube.Trans_m[2]=z; }
  	}

  	// RGB – masz sValue_RGB; jeśli puste, spróbuj po nazwie; domyślnie 128 128 128
  	{
    		std::string rgbStr = (sValue_RGB ? sValue_RGB : "");
   		if (rgbStr.empty()) rgbStr = getAttr("RGB");
    		if (!rgbStr.empty()) {
      			std::istringstream is(rgbStr);
      			int r,g,b; if (is >> r >> g >> b) { cube.RGB[0]=r; cube.RGB[1]=g; cube.RGB[2]=b; }
    		}
	}

	std::cout << "Dodano Cube: " << cube.Name
        	<< "  Shift=(" << cube.Shift[0] << "," << cube.Shift[1] << "," << cube.Shift[2] << ")"
            	<< "  Scale=(" << cube.Scale[0] << "," << cube.Scale[1] << "," << cube.Scale[2] << ")"
            	<< "  RotXYZ_deg=(" << cube.RotXYZ_deg[0] << "," << cube.RotXYZ_deg[1] << "," << cube.RotXYZ_deg[2] << ")"
            	<< "  Trans_m=(" << cube.Trans_m[0] << "," << cube.Trans_m[1] << "," << cube.Trans_m[2] << ")"
            	<< "  RGB=(" << cube.RGB[0] << "," << cube.RGB[1] << "," << cube.RGB[2] << ")" << std::endl;

	_rConfig.AddCube(cube);

 // Tu już został wstawiony własny kod
